...
template <uint64_t CAPACITY> class TBBAllocator {
public:
  TBBAllocator() {
    pool_aligned4_ = tbb::scalable_allocator<uint32_t>().allocate(CAPACITY / 4);
    memset(pool_aligned4_, 0x00, CAPACITY / 4);
    count_aligned4_ = tbb::scalable_allocator<std::atomic<uint32_t>>().allocate(sizeof(std::atomic<uint32_t>));
  ...
  void free_all() {
    tbb::scalable_allocator<uint32_t>().deallocate(pool_aligned4_, CAPACITY / 4);
    tbb::scalable_allocator<std::atomic<uint32_t>>().deallocate(count_aligned4_, sizeof(std::atomic<uint32_t>));
...
template <uint64_t CAPACITY> class KeyTBBAllocator {
public:
  KeyTBBAllocator() {
    small_pool_aligned8_ = tbb::scalable_allocator<uint64_t>().allocate(CAPACITY / 24);
    memset(small_pool_aligned8_, 0x00, CAPACITY / 24);
    small_count_aligned8_ = tbb::scalable_allocator<std::atomic<uint32_t>>().allocate(sizeof(std::atomic<uint32_t>));
    memset(small_count_aligned8_, 0x00, sizeof(std::atomic<uint32_t>));

    medium_pool_aligned8_ = tbb::scalable_allocator<uint64_t>().allocate(CAPACITY / 24);
    memset(medium_pool_aligned8_, 0x00, CAPACITY / 24);
    medium_count_aligned8_ = tbb::scalable_allocator<std::atomic<uint32_t>>().allocate(sizeof(std::atomic<uint32_t>));
    memset(medium_count_aligned8_, 0x00, sizeof(std::atomic<uint32_t>));

    large_pool_aligned8_ = tbb::scalable_allocator<uint64_t>().allocate(CAPACITY / 24);
    memset(large_pool_aligned8_, 0x00, CAPACITY / 24);
    large_count_aligned8_ = tbb::scalable_allocator<std::atomic<uint32_t>>().allocate(sizeof(std::atomic<uint32_t>));
  ...
  void free_all() {
    tbb::scalable_allocator<uint64_t>().deallocate(small_pool_aligned8_, CAPACITY / 24);
    small_pool_aligned8_ = nullptr;
    tbb::scalable_allocator<uint64_t>().deallocate(medium_pool_aligned8_, CAPACITY / 24);
    medium_pool_aligned8_ = nullptr;
    tbb::scalable_allocator<uint64_t>().deallocate(large_pool_aligned8_, CAPACITY / 24);
    large_pool_aligned8_ = nullptr;
    tbb::scalable_allocator<std::atomic<uint32_t>>().deallocate(small_count_aligned8_, sizeof(std::atomic<uint32_t>));
    small_count_aligned8_ =nullptr;
    tbb::scalable_allocator<std::atomic<uint32_t>>().deallocate(medium_count_aligned8_, sizeof(std::atomic<uint32_t>));
    medium_count_aligned8_ =nullptr;
    tbb::scalable_allocator<std::atomic<uint32_t>>().deallocate(large_count_aligned8_, sizeof(std::atomic<uint32_t>));
  ...
  uint32_t allocated() const {
    return (*small_count_aligned8_).load() + (*medium_count_aligned8_).load()*4 + (*large_count_aligned8_).load()*32;
  }
