...
class UMDynamicAllocator {
  public:
    UMDynamicAllocator() {
      CHECK_ERROR(gutil::UMAlloc(d_pool_aligned4_, CAPACITY / 4));
      CHECK_ERROR(gutil::UMSet(d_pool_aligned4_, 0x00, CAPACITY / 4));
      CHECK_ERROR(gutil::UMAlloc(d_count_aligned4_, 1));
      CHECK_ERROR(gutil::UMSet(d_count_aligned4_, 0x00, 1));

      //print address of d_pool_aligned4_ and d_count_aligned4_
      printf("d_pool_aligned4_ address: %p\n", d_pool_aligned4_);
      printf("d_count_aligned4_ address: %p\n", d_count_aligned4_);
    }
    ~UMDynamicAllocator() {
    }
    ...
    UMDynamicAllocator &operator=(const UMDynamicAllocator &rhs) {
    ...
    __forceinline__ uint32_t * get_pool_ptr() const {
...
class TBBAllocator {
 public:
  TBBAllocator() {
    ...
    memset(pool_aligned4_, 0x00, CAPACITY / 4 *sizeof(uint32_t));
...
template <uint64_t CAPACITY>
class UMKeyDynamicAllocator {
 public:
  UMKeyDynamicAllocator() {
    CHECK_ERROR(gutil::UMAlloc(d_small_pool_aligned8_, CAPACITY / 24));
    CHECK_ERROR(gutil::UMSet(d_small_pool_aligned8_, 0x00, CAPACITY / 24));
    CHECK_ERROR(gutil::UMAlloc(d_small_count_aligned8_, 1));
    CHECK_ERROR(gutil::UMSet(d_small_count_aligned8_, 0x00, 1));

    CHECK_ERROR(gutil::UMAlloc(d_medium_pool_aligned8_, CAPACITY / 24));
    CHECK_ERROR(gutil::UMSet(d_medium_pool_aligned8_, 0x00, CAPACITY / 24));
    CHECK_ERROR(gutil::UMAlloc(d_medium_count_aligned8_, 1));
    CHECK_ERROR(gutil::UMSet(d_medium_count_aligned8_, 0x00, 1));

    CHECK_ERROR(gutil::UMAlloc(d_large_pool_aligned8_, CAPACITY / 24));
    CHECK_ERROR(gutil::UMSet(d_large_pool_aligned8_, 0x00, CAPACITY / 24));
    CHECK_ERROR(gutil::UMAlloc(d_large_count_aligned8_, 1));
    CHECK_ERROR(gutil::UMSet(d_large_count_aligned8_, 0x00, 1));
  }
  ~UMKeyDynamicAllocator() {
    // TODO: free them
  }
  ...
  UMKeyDynamicAllocator &operator=(const UMKeyDynamicAllocator &rhs) {
...
class KeyTBBAllocator {
 public:
  KeyTBBAllocator() {
    ...
    memset(small_pool_aligned8_, 0x00, CAPACITY / 24 * sizeof(uint64_t));
    ...
    tbb::scalable_allocator<uint64_t>().allocate(CAPACITY / 24);
    memset(medium_pool_aligned8_, 0x00, CAPACITY / 24 * sizeof(uint64_t));
    ...
    large_pool_aligned8_ =
        tbb::scalable_allocator<uint64_t>().allocate(CAPACITY / 24);
    memset(large_pool_aligned8_, 0x00, CAPACITY / 24 * sizeof(uint64_t));
