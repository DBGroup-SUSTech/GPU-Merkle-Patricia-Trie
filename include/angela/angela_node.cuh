#pragma once
// only have one type of AngelaNode
...
{                         // 192 bytes
  ...
  // update
  AngelaNode *parent = nullptr;
  int child_num=0;
  // fields for hash update
  int add_parent_flag = 0;
  int visit_count = 0;

  __forceinline__ __device__ void make_hash_input(uint64_t *temp_buffer)
  {
    uint64_t *p = temp_buffer;
    for (int i = 0; i < 16; ++i)
    {
      AngelaNode *child = childs[i];
      if (child != nullptr)
      {
        memcpy(p, child->hash, 4);
        p += 4;
      }
    }
  }
  ...
  __host__ void update_hash_cpu(uint8_t *tmp_buffer)
  {
    ...
    if (has_value)
    {
      ...
      // printf("Current AngelaNode has value\n\tdata = ");
